...
_action("CONVERT_LABEL","Convert to numerical");
...
_type("POLAR","Polar plot",{topvalue:1,leftvalue:2,bottomlabel:1,display:3,options:2,menu:"angle"});
...
_type("TEST","Analysis of variance",{ivalues:1,leftlabel:1,menu:"test"});
...
_type("REGRES","Linear regression",{ivalues:1,leftvalue:1,options:2,menu:"regr"});
...
img.src = "./checker.png";
img.onload = function(){
	dotted = ctx.createPattern(img,"repeat");
	}
...
var offset = GINFO[graph.type].ilabels ? dy*graph.ilabels.length :
			GINFO[graph.type].ivalues ? dy*graph.ivalues.length : 0;

var y = graph.y+graph.hbar+5+offset+dy+dy;
for(var k=0;k<=n;k++)	
...
var pvalue = chi2inv(0.001,100,kruskal,ng-1);
var cv = critchi(0.05,ng-1);
...
graph._z.cv = Finv(0.0001,1000,0.05,graph._z.dof1,graph._z.dof2);
graph._z.pvalue = Fspin(graph._z.t2,graph._z.dof1,graph._z.dof2);
...
graph._z.nb = nb;
graph._z.dof = nt-1;
graph._z.cv =  critchi(0.05,nt-1);
graph._z.pvalue = chi2inv(0.001,100,q,nt-1);
...
var name2 = getGraphLabel(graph,"leftlabel");
	...
	max *= 1.2;
	var pvalue = chi2inv(0.00001,max,graph._z.chi2,dof);
...
if(inRect(pt,graph.x+graph.w-85,ytop,20,20))
	{
	return DRAG_SET;
	}
...
ctx.fillStyle = "#FFFFFF";
drawRightArrow(ctx,graph.x+graph.w-75,ytop+10);
...
var dof = ng-1;
var level = 0.05;
var cv = critchi(level,dof);

var max = 2*Math.max(t,cv);

var pvalue = chi2inv(0.01,max,t,dof);
...
var pvalue = Fspin(F,ninter,nintra)	
...
switch(graph.test)
	{
	/*
	case TEST.FISHER: buildFisherTable(graph); break;
	case TEST.BARTLETT : buildBartlettTable(graph); break;
	case TEST.LEVENE: buildLeveneTable(graph); break;
	case TEST.BROWN: buildLeveneTable(graph); break;
	case TEST.BOXM: buildBoxmTable(graph); break;
	*/
	}
}
...
function buildFisherTable(graph)
{
setTableName("Fisher test");

var row = 1;
table(row,1,"Fisher test");
...
var dof = graph._z.ninter+","+graph._z.nintra;
row++;
...
table(row,2,Math.round(cv*10000)/10000);
table(row,3,"(\u03B1="+level+")");
...
table(row,2,Math.round(F*10000)/10000);
table(row,3,"(pvalue="+(Math.round(pvalue*10000)/10000)+")");
...
console.log("COEF");
for(var i=0;i<n;i++)
	console.log(A[i]+" "+JSON.stringify(list[i]));
*/
...
var cv = Finv(0.0001,max,0.05,dof1,dof2);

var pvalue = Fspin(F,dof1,dof2);
...
if((graph.ivalues.length>0)&&(graph.ivalue1>=0))
	...
	if(option==0)
		...
		ctx.fillStyle = "#FF0000";
		if(pvalue<0.05)	
			multiText(ctx,["#000000",values[graph.ivalue1],
				"#FF0000" ," is dependent ",
...
ctx.fillStyle = "#FFFFFF";
ctx.fillRect(graph.x+graph.w-130,graph.y+graph.hbar+1,130,
...
if(isnum)
	...
	// convert as is
	for(var i=0;i<lrecords.length;i++)
		vrecords[i].push(Number(lrecords[i][index].replace(",",".")));
	}
else
	{	
	// convert to index 
	...
	for(var i=0;i<lrecords.length;i++)
		vrecords[i].push(set[lrecords[i][index]]);
	}
...
if(faction==DRAG_SIDEBAR)
	...
	if(inDustbinIcon(ptmove))
		...
		{
		action = labelInUse(labelindex) ? DONT_REMOVE:CONVERT_LABEL;
		}
	else if(i<0)
		{
		action = DRAG_LABEL;
...
if(action==SET_VALUEJ)
	...
	var graph = graphs[graphindex]
	ctx.fillStyle = GRAY;
	var offset = GINFO[graph.type].ilabels ? (SLOTH+5)*graph.ilabels.length :
				GINFO[graph.type].ivalues ? (SLOTH+5)*graph.ivalues.length : 0;
	var y = graph.y+graph.hbar+5+offset+50+25*destvalueindex;
	ctx.fillRect(graph.x+graph.w-105,y,100,20)	;
	}
...
if(GINFO[graph.type].jvalues)
	{
	var offset = GINFO[graph.type].ivalues ? dy*graph.ivalues.length : 
				GINFO[graph.type].ilabels ? dy*graph.ilabels.length : 0;
	var y = graph.y+graph.hbar+5+offset+dy+dy;
...
return 1-dnormal(x)*( b1*t+b2*t*t+b3*t*t*t+b4*t*t*t*t+b5*t*t*t*t*t);
}
...
for(var i=0;i<=100;i++)
	{
	var d = dnormal(min+(max-min)*i/100);
	if(d>dmax) dmax = d;
	}
